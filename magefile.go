...
const formulaTemplate = `class Aictl < Formula
    ...
    on_linux do
        if Hardware::CPU.arm?
            url "https://github.com/go-zen-chu/aictl/releases/download/v%[1]s/aictl_Linux_arm64.tar.gz"
            sha256 "{{.ChecksumSHA256LinuxArm64}}"
        else
            url "https://github.com/go-zen-chu/aictl/releases/download/v%[1]s/aictl_Linux_x86_64.tar.gz"
            sha256 "{{.ChecksumSHA256LinuxX86_64}}"
        end
    end
